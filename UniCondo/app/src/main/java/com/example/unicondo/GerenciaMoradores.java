...
public class GerenciaMoradores extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gerencia_moradores);
        ...
        btnRemover.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                SparseBooleanArray sp = lstCondominos.getCheckedItemPositions();
                ...
                for (int i = 0; i < sp.size(); i++) {
                    if (sp.valueAt(i) == true) {
                        numSelecionados++;
                        Classe_Moradores user = (Classe_Moradores) lstCondominos.getItemAtPosition(i);
                        // Or:
                        // String s = ((CheckedTextView) listView.getChildAt(i)).getText().toString();
                        //
                        //String s = user.getNome();
                        //sb = sb.append(" " + s);
                        removerMoradores(user.getId());
                    }
                }
                if (numSelecionados == 0){
                    ...
                    if (tipoUsuario == 1) {
                        Toast.makeText(esseContexto, "Todos os condôminos selecionados foram removidos!", Toast.LENGTH_LONG).show();
                        finish();
                    } else if (tipoUsuario == 2) {
                        Toast.makeText(esseContexto, "Todos os síndicos selecionados foram removidos!", Toast.LENGTH_LONG).show();
                        finish();
                    }
                }
            }
                ...
                new Response.Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        try {
                            ...
                            ArrayAdapter<Classe_Moradores> adapter = new ArrayAdapter<Classe_Moradores>(
                                    esseContexto, android.R.layout.simple_list_item_multiple_choice, listaDeMoradores);
                            //adapter.notifyDataSetChanged();
                            lstCondominos.setAdapter(adapter);
                            //lstCondominos.requestLayout();
                            //((ArrayAdapter) lstCondominos.getAdapter()).notifyDataSetChanged();
...
/*
    ...
    public void removerMoradores(int id){

        if (tipoUsuario == 1){
            urlWebService = "https://api-unicondo.leonardo-bezerra.dev/condominiumium-tenants/" + id;
        } else if (tipoUsuario == 2){
            urlWebService = "https://api-unicondo.leonardo-bezerra.dev/condominium-managers/" + id;
        }
